...
(
    ...
    const int64_t *__restrict__ Ah,  // A->h [0..A->nvec-1]: list of vectors
    const int64_t *__restrict__ Ap,  // A->p [0..A->nvec  ]: pointers to vectors
    const int64_t avlen,            // A->vlen
    int64_t *__restrict__ pleft,     // look only in A->h [pleft..pright]
...
//  const int64_t nvec,             // A->nvec: number of vectors
    ...
    int64_t *__restrict__ pstart,    // start of vector: Ap [k]
    int64_t *__restrict__ pend       // end of vector: Ap [k+1]
)
{
    if (A_is_hyper)
    {
